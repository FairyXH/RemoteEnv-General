use remote_env_platform_windows::bluetooth::{BluetoothCollector, NativeBleScanner, NativeClassicBluetoothScanner};

fn main() {
    let collector = BluetoothCollector::new(NativeBleScanner::new(), NativeClassicBluetoothScanner::new());
    ...
    match collector.scan_once() {
        Ok(event) => {
            ...
            println!("Classic Bluetooth available: {}", event.data["classic_available"]);
