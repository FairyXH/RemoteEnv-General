...
The production desktop path passes `NativeWlanProvider` into `RuntimeSupervisor::start_with_collector`. The worker uses the configured `scan_interval_seconds`, always collects every WLAN interface (no enable/disable switch), isolates blocking WLAN calls, applies a 120-second timeout, emits status transitions, and stops with the Runtime. On scan error it retries with exponential backoff capped at 120 seconds. Configuration updates are forwarded without process restart.
