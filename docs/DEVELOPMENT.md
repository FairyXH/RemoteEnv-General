...
Run `cargo run -p remote-env-platform-windows --example windows_bluetooth_scan`. The probe performs one unified BLE + Classic scan and prints only capability, aggregate count, and duration.
The probe now also prints separate BLE and Classic observation counts. Runtime production uses a reusable BluetoothCollector with parallel source scans, reliable BLE callback delivery, extended advertisements, and a 120-second rolling deduplicated snapshot. Wi-Fi and Bluetooth both collect continuously (no enable/disable switch), each producing its own `CollectorEvent`; the Runtime uploads every source as an independent server-canonical envelope. UI details come from the pushed source snapshots, not only manual scan commands. The combined payload uses top-level `devices` for server/dashboard compatibility and puts Wi-Fi data in the same envelope. The local Phase 2-B fixture is `crates/core/tests/phase2b.rs`. It covers one Bluetooth event through Runtime sequence allocation, `upload_deliveries`, local WebSocket ACK completion, continuous collection across config updates (interval change reuses one worker without restart), and two-server target-scoped ACK isolation. Its recovery path closes one server after an unacknowledged upload and verifies the same envelope, device ID, data type, sequence, and payload are resent. No real backend credentials are used.
