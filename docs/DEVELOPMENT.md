...
The probe now also prints separate BLE and Classic observation counts. Runtime production uses a reusable BluetoothCollector with parallel source scans, reliable BLE callback delivery, extended advertisements, and a 120-second rolling deduplicated snapshot. When Wi-Fi and Bluetooth are both enabled, the Runtime uploads one combined `bluetooth` envelope per timestamp-compatible pair; UI details come from the pushed source snapshots, not only manual scan commands. The combined payload uses top-level `devices` for server/dashboard compatibility and puts Wi-Fi data in the same envelope. The local Phase 2-B fixture is `crates/core/tests/phase2b.rs`. It covers one Bluetooth event through Runtime sequence allocation, `upload_deliveries`, local WebSocket ACK completion, dynamic `bluetooth_enabled` enable/disable without a second worker, and two-server target-scoped ACK isolation. Its recovery path closes one server after an unacknowledged upload and verifies the same envelope, device ID, data type, sequence, and payload are resent. No real backend credentials are used.
