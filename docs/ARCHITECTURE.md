...
Production path: `Wi-Fi/Bluetooth continuous collectors -> latest snapshots -> RuntimeSupervisor -> per-source CollectorEvent (data_type="wifi" and/or data_type="bluetooth") -> UploadDispatcher -> upload_deliveries -> DispatcherSupervisor -> ServerWorker(s) -> independent WebSocket -> RemoteEnvServer`.
...
Windows now has one unified Bluetooth collector: `BluetoothCollector -> BLE scanner + Classic Bluetooth scanner -> BluetoothSnapshot`. BLE uses WinRT `BluetoothLEAdvertisementWatcher` with active and extended advertisements, reliable callback collection, `Stop`, `RemoveReceived`, structured fields, and raw `DataSections()` preservation. Classic uses the native Bluetooth inquiry APIs. Runtime owns one Bluetooth worker, uses `scan_interval_seconds`, always collects (no enable/disable switch), retains a 120-second deduplicated rolling observation set, and routes each source snapshot into its own server-canonical envelope and independent ServerWorkers. A/B local fixtures cover identical envelopes, ACK isolation, disconnect recovery, and sequence preservation. Unknown raw AD sections use `source`, `ad_type`, and uppercase `data_hex`; scan responses are distinguished when Windows reports them. Real Windows counts are environment-dependent and are not used as the sole acceptance criterion.

`RuntimeSupervisor::start_with_collector` starts optional platform scan callbacks on dedicated worker runtimes. Wi-Fi and Bluetooth scan continuously on the configured interval and stop checks prevent new work after shutdown. Each source produces its own `CollectorEvent` and the Runtime uploads every source as an independent server-canonical envelope (`data_type="wifi"` and `data_type="bluetooth"`), because the server selects the data schema by `data_type`; merging them into one Bluetooth envelope would hide Wi-Fi data from the server's per-type latest-data/statistics views. A bounded 120-second timeout converts a hung scan into an Error state; the Runtime continues and later cycles can retry. Runtime config updates apply enablement and interval without restarting the process.
