...
The runtime now keeps Wi-Fi and Bluetooth collectors running independently but persists/upload events as one `data_type="environment"` envelope when both collectors are enabled. The envelope contains one `captured_at_ms`, `data.wifi`, and `data.bluetooth`; a pair is emitted only when the two source event timestamps are within the configured scan interval plus 15 seconds. Sequence and ACK count therefore advance once per combined packet, not once per radio.
