...
The fixture has since added explicit A/B delivery-state assertions, heartbeat/auth-frame observation, Single A -> Multi A+B, Multi A+B -> Single B, profile removal cancellation, profile URL/token replacement, rate-limit isolation, and missing-pong reconnect coverage. The standalone Python WebSocket smoke was attempted with the supplied runtime-only environment variables after confirming `websockets` 16.1.1 is installed. It failed before TLS/WebSocket handshake because Python also received Windows DNS error `socket.gaierror: [Errno 11001] getaddrinfo failed`; no real auth or upload was sent. The temporary script and environment variables were removed.
