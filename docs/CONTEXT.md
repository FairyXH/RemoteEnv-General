...
The fixture has since added explicit A/B delivery-state assertions, heartbeat/auth-frame observation, Single A -> Multi A+B, Multi A+B -> Single B, profile removal cancellation, profile URL/token replacement, rate-limit isolation, and missing-pong reconnect coverage. Detached fixture task failure propagation remains a test-harness limitation.
