...
Local Phase 1.75-C integration tests exercise two independent listeners, same-envelope delivery, target-local ACK/reconnect recovery, profile replacement/removal, rate-limit classification, heartbeat/pong observation, and permanent auth failure. Missing pong causes the worker heartbeat monitor to classify the connection as transport-failed and enter reconnect. A dedicated missing-pong integration test remains pending.
