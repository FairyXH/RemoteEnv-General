...
Sequence recovery is also timestamp-based: after `sequence_rejected`, the worker advances the affected persisted delivery baseline to `max(current Unix milliseconds, rejected_sequence + 1)` before retrying. All newly allocated sequences use the same Unix-millisecond floor, including the compatibility queue API; no path may generate counter values starting at `1`.

For server `rate_limited`, the worker does not busy-loop: it recovers the target's in-flight delivery, keeps the worker in the infinite reconnect loop, and waits through the per-worker capped exponential backoff `1s -> 2s -> 4s -> 8s -> 16s -> 32s -> 60s -> 60s...`. The backoff is reset only after a connection attempt reaches successful authenticated/ready operation; stopping the Runtime interrupts the wait immediately.
