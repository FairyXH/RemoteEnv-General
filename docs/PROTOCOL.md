...
- Wi-Fi network records include their own positive Unix-millisecond `timestamp`; Bluetooth records include `address_type` (currently `unknown` where the Windows API does not expose a reliable public/random classification). Bluetooth transport `mode` remains a deliberately documented extension for the UI, while the protocol-facing technology value is canonical.
