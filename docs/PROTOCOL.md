...
- When both Wi-Fi and Bluetooth collection are enabled, the production runtime sends exactly one combined envelope per compatible snapshot pair using `data_type: "bluetooth"`: `data: {"captured_at_ms": ..., "scan_started_at": ..., "scan_finished_at": ..., "is_enabled": ..., "technology":"unknown", "devices": [...], "wifi": <WiFiData>, "bluetooth": <BluetoothData>}`. The Bluetooth server schema (`scan_started_at`, `scan_finished_at`, `technology`, `is_enabled`, `devices`) is therefore satisfied at the top level; Wi-Fi and nested Bluetooth payloads remain as extension fields. BLE and Classic observations are unified in `devices` and distinguished per device by `mode` (`ble`, `classic`, or `dual`). Both source snapshots must be from events no more than 15 seconds apart; otherwise the older side is discarded and the runtime waits for a fresh pair. One sequence, one delivery, and one ACK represent the combined packet.
