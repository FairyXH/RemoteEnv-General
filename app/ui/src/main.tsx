...
function App() {
  ...
  const toggle = async () => { if (busyRef.current) return; busyRef.current = true; setBusy("runtime"); const shouldStart = !status.collection_running; try { const next = await invoke<RuntimeStatus>(shouldStart ? "start_runtime" : "stop_runtime"); setStatus(next); setNotice(shouldStart ? "采集服务已启动。" : "采集服务已停止，服务器连接保持独立运行。"); } catch (error) { window.alert(`采集服务操作失败：${String(error)}`); } finally { busyRef.current = false; setBusy(null); } };
  const connect = async (id: string) => { if (busyRef.current || !serverActionAllowed(id)) return; busyRef.current = true; markServerAction(id); setBusy(`connect:${id}`); setNotice("正在建立持久连接..."); try { await invoke<RuntimeStatus>("connect_server_profile", { id }); setNotice("服务器连接已启动，采集服务仍保持停止。"); } catch (error) { window.alert(`服务器连接失败：${String(error)}`); } finally { busyRef.current = false; setBusy(null); } };
  const disconnect = async (id: string) => { if (busyRef.current || !serverActionAllowed(id)) return; busyRef.current = true; markServerAction(id); setBusy(`disconnect:${id}`); try { await invoke<RuntimeStatus>("disconnect_server_profile", { id }); setConfig(current => ({ ...current, server_profiles: current.server_profiles.map(server => server.id === id ? { ...server, enabled: false } : server) })); setNotice("服务器已断开。"); } catch (error) { window.alert(`服务器断开失败：${String(error)}`); } finally { busyRef.current = false; setBusy(null); } };
