...
function App() {
  ...
  return <main className="shell">
    ...
    <section className="runtime"><div><h2>采集服务</h2><span>{stateText(status.connection)} · 待上传 {status.pending} · 发送中 {status.in_flight}</span></div><button className="primary" disabled={busy !== null} onClick={toggle}>{status.connection === "Stopped" || status.connection === "Reconnecting" ? (busy === "runtime" ? "启动中..." : "启动") : (busy === "runtime" ? "停止中..." : "停止")}</button></section>
