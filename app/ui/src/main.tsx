...
type RuntimeStatus = { connection: string; wifi: string; bluetooth: string; pending: number; in_flight: number; blocked: number; uploaded: number; servers: Array<{ profile_id: string; connection: string; heartbeat_alive: boolean; last_heartbeat_ms: number | null; last_error?: string | null; pending: number; in_flight: number; blocked: number }>; wifi_runtime: Scan; bluetooth_runtime: Scan };
...
function App() {
  ...
  const toggle = async () => { if (busy) return; setBusy("runtime"); const shouldStart = status.connection === "Stopped" || status.connection === "Reconnecting" || status.connection === "Disconnected"; try { if (shouldStart) { const next = await invoke<RuntimeStatus>("start_runtime"); setStatus({ ...next, connection: next.connection === "Stopped" || next.connection === "Disconnected" ? "Connecting" : next.connection }); setNotice("采集服务已启动，正在持续连接服务器。"); } else { const next = await invoke<RuntimeStatus>("stop_runtime"); setStatus(next); } } catch (error) { window.alert(`采集服务启动失败：${String(error)}`); } finally { setBusy(null); } };
  const connect = async (id: string) => { if (busy || !serverActionAllowed(id)) return; markServerAction(id); setBusy(`connect:${id}`); setNotice("正在建立持久连接..."); try { const next = await invoke<RuntimeStatus>("connect_server_profile", { id }); setStatus({ ...next, connection: next.connection === "Stopped" || next.connection === "Disconnected" ? "Connecting" : next.connection }); setNotice("服务器连接已启动，正在持续更新状态。"); } catch (error) { window.alert(`服务器连接失败：${String(error)}`); } finally { setBusy(null); } };
  const disconnect = async (id: string) => { if (busy || !serverActionAllowed(id)) return; markServerAction(id); setBusy(`disconnect:${id}`); try { const next = await invoke<RuntimeStatus>("disconnect_server_profile", { id }); setStatus(next); setConfig(current => ({ ...current, server_profiles: current.server_profiles.map(server => server.id === id ? { ...server, enabled: false } : server) })); setNotice("服务器已断开。"); } catch (error) { window.alert(`服务器断开失败：${String(error)}`); } finally { setBusy(null); } };
  const toggleServer = async (server: Server) => { if (busy || !serverActionAllowed(server.id)) return; markServerAction(server.id); setBusy(`toggle:${server.id}`); try { const next = await invoke<RuntimeStatus>("set_server_enabled", { id: server.id, enabled: !server.enabled }); setStatus(next); setConfig(current => ({ ...current, active_server_id: !server.enabled ? server.id : current.active_server_id, server_profiles: current.server_profiles.map(item => item.id === server.id ? { ...item, enabled: !server.enabled } : item) })); setNotice(!server.enabled ? "服务器已启用，正在连接并上传。" : "服务器已断开。"); } catch (error) { window.alert(`服务器切换失败：${String(error)}`); } finally { setBusy(null); } };
