...
function App() {
  ...
  const toggle = async () => { if (busy) return; setBusy("runtime"); try { const next = await invoke<RuntimeStatus>(status.connection === "Stopped" ? "start_runtime" : "stop_runtime"); setStatus(next); } catch (error) { setNotice(String(error)); } finally { setBusy(null); } };
  const connect = async (id: string) => { if (busy) return; setBusy(`connect:${id}`); setNotice("正在建立持久连接..."); try { await invoke("connect_server_profile", { id }); setNotice("服务器连接已启动，状态将持续更新。"); } catch (error) { setNotice(String(error)); } finally { setBusy(null); } };
  const scan = async (kind: "wifi" | "bluetooth") => { if (busy) return; setBusy(kind); setNotice(`正在扫描${kind === "wifi" ? " Wi-Fi" : "蓝牙"}...`); try { const event = await invoke<{ data: unknown }>(kind === "wifi" ? "scan_wifi_now" : "scan_bluetooth_now"); setScanData(current => ({ ...current, [kind]: event.data })); const value = event.data as Record<string, unknown>; const items = Array.isArray(value.networks) ? value.networks : Array.isArray(value.observations) ? value.observations : []; setScanSummary(current => ({ ...current, [kind]: { ...(current[kind]), ...(kind === "wifi" ? { network_count: items.length } : { device_count: items.length }), last_scan_ms: Date.now(), duration_ms: Number(value.scan_duration_ms ?? 0) } })); setNotice(`${kind === "wifi" ? "Wi-Fi" : "蓝牙"}扫描完成。`); } catch (error) { setNotice(String(error)); } finally { setBusy(null); } };
  ...
  return <main className="shell">
    ...
    <section className="runtime"><div><h2>运行时</h2><span>{stateText(status.connection)} · 待上传 {status.pending} · 发送中 {status.in_flight}</span></div><button className="primary" disabled={busy !== null} onClick={toggle}>{status.connection === "Stopped" || status.connection === "Reconnecting" ? (busy === "runtime" ? "启动中..." : "启动") : (busy === "runtime" ? "停止中..." : "停止")}</button></section>
