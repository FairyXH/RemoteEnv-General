...
function App() {
  ...
  const toggleServer = async (server: Server) => { if (busy || !serverActionAllowed(server.id)) return; markServerAction(server.id); setBusy(`toggle:${server.id}`); try { const next = await invoke<RuntimeStatus>("set_server_enabled", { id: server.id, enabled: !server.enabled }); setStatus(next); setConfig(current => ({ ...current, server_profiles: current.server_profiles.map(item => item.id === server.id ? { ...item, enabled: !server.enabled } : item) })); setNotice(!server.enabled ? "服务器已启用，正在连接并上传。" : "服务器已断开。"); } catch (error) { window.alert(`服务器切换失败：${String(error)}`); } finally { setBusy(null); } };
