...
type Config = { device_id: string; server_mode: "Single" | "Multi"; active_server_id: string | null; server_profiles: Server[]; scan_interval_seconds: number; upload_interval_seconds: number };
...
type StateInfo = { config_db_path: string; config_db_bytes: number; state_cache_db_path: string; state_cache_db_bytes: number };
...
const emptyConfig: Config = { device_id: "", server_mode: "Single", active_server_id: null, server_profiles: [], scan_interval_seconds: 1, upload_interval_seconds: 30 };
...
function App() {
  ...
  const refreshStateInfo = React.useCallback(() => { invoke<StateInfo>("get_state_info").then(setStateInfo).catch(() => undefined); }, []);
  React.useEffect(() => { refreshStateInfo(); const timer = window.setInterval(refreshStateInfo, 15000); return () => window.clearInterval(timer); }, [refreshStateInfo]);
  const cleanupCache = async () => { if (cleaning) return; setCleaning(true); try { const result = await invoke<{ removed_rows: number; state_cache_db_bytes: number }>("cleanup_state_cache"); setNotice(`状态缓存清理完成：删除 ${result.removed_rows} 条已完成记录，当前缓存大小 ${formatBytes(result.state_cache_db_bytes)}。`); refreshStateInfo(); } catch (error) { setNotice(`状态缓存清理失败：${String(error)}`); } finally { setCleaning(false); } };
